...
class TestBaselineSparkCoreBudget:
    ...
    def _run(self, num_workers=2, cores_override=None):
        ...
        with (
            patch(
                "mpj_spark.applications.baseline_spark.build_spark_session",
...
class TestBaselineKmeansCoreBudget:
    ...
    def _run(self, num_workers=2, cores_override=None, baseline_threads=None, total_cores=22):
        ...
        with (
            patch(
                "mpj_spark.applications.baseline_kmeans.build_spark_session",
                return_value=fake_spark,
            ),
            patch("mpj_spark.config.TOTAL_CORES", total_cores),
            patch(
                "mpj_spark.applications.baseline_kmeans.VectorAssembler",
                return_value=assembler_inst,
            ),
            patch(
                "mpj_spark.applications.baseline_kmeans.KMeans",
...
class TestBaselineLogregCoreBudget:
    ...
    def _run(
        ...
        with (
            patch("mpj_spark.applications.baseline_logreg.SparkSession") as mock_ss,
            patch("mpj_spark.config.TOTAL_CORES", 20),
            patch(
                "mpj_spark.applications.baseline_logreg.VectorAssembler",
                return_value=assembler_inst,
            ),
            patch(
