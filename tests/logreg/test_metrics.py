...
class TestRecordEpoch:
    ...
    def test_appends_correct_keys(self, tmp_path):
        ...
        c.record_epoch(epoch=1, spark_time_s=0.1, sync_time_s=0.05,
                       epoch_time_s=0.15, grad_norm=0.5,
                       global_loss=0.693, weight_norm=0.3)
    ...
    def test_epoch_number_stored_correctly(self, tmp_path):
        c = _make_collector(tmp_path)
        c.record_epoch(epoch=5, spark_time_s=0.2, sync_time_s=0.1,
                       epoch_time_s=0.3, grad_norm=0.1,
                       global_loss=0.5, weight_norm=0.2)
    ...
    def test_values_are_rounded(self, tmp_path):
        ...
        """
        c = _make_collector(tmp_path)
        c.record_epoch(epoch=1, spark_time_s=0.123456789,
                       sync_time_s=0.0, epoch_time_s=0.0,
                       grad_norm=1.123456789123,
                       global_loss=0.5, weight_norm=0.0)
...
class TestRecordRun:
    ...
    def test_stores_all_run_fields(self, tmp_path):
        ...
        c.record_run(total_time_s=12.34, epochs_run=10,
                     converged=True, dataset_size=50000,
                     num_ranks=3, learning_rate=0.01, tol=1e-4)
    ...
    def test_total_time_rounded_to_4dp(self, tmp_path):
        c = _make_collector(tmp_path)
        c.record_run(total_time_s=3.141592653, epochs_run=1,
                     converged=False, dataset_size=100,
                     num_ranks=1, learning_rate=0.1, tol=0.01)
...
class TestToCsv:
    ...
    def test_csv_values_match_recorded_data(self, tmp_path):
        c = _make_collector(tmp_path)
        c.record_epoch(epoch=1, spark_time_s=0.5, sync_time_s=0.25,
                       epoch_time_s=0.75, grad_norm=0.333,
                       global_loss=0.6, weight_norm=1.0)
        path = c.to_csv()
        with open(path, newline="", encoding="utf-8") as fh:
...
class TestToJson:
    ...
    def test_json_file_created(self, tmp_path):
        c = _make_collector(tmp_path)
        c.record_run(total_time_s=1.0, epochs_run=3, converged=False,
                     dataset_size=100, num_ranks=1,
    ...
    def test_json_is_valid_and_matches_run(self, tmp_path):
        ...
        c.record_run(total_time_s=5.0, epochs_run=10, converged=True,
                     dataset_size=200, num_ranks=3,
                     learning_rate=0.05, tol=1e-3)
        path = c.to_json()
...
class TestAggregateAcrossRanks:
    ...
    def test_aggregated_csv_contains_all_rank_rows(self, tmp_path):
        ...
        out = LogRegMetricsCollector.aggregate_across_ranks(
            output_dir=str(tmp_path), num_ranks=2
        )
    ...
    def test_aggregated_csv_has_rank_column(self, tmp_path):
        ...
        out = LogRegMetricsCollector.aggregate_across_ranks(
            output_dir=str(tmp_path), num_ranks=1
        )
        with open(out, newline="", encoding="utf-8") as fh:
            reader = csv.DictReader(fh)
