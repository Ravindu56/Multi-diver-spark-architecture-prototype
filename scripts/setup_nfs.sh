...
# EXPORT_DIR to the libvirt VM subnet. Worker nodes mount that export over
# /etc/fstab at MOUNT_DIR, which is the identical path - so the root
# coordinator and every Spark-driver container see one shared filesystem
# layout (the cloud-native equivalent of Lustre in the MPJ-Spark reference
# architecture).
#
# The manager must NOT NFS-mount its own export: the export directory is
# already the shared path natively, and self-mounting wedges exportfs
# (requires fsid=) and creates a systemd ordering cycle when restarting
# nfs-kernel-server with the mount active.
#
...
# ---- 2. fstab mount on workers (uniform path for P5-03 bind mounts) ----
...
for attempt in 1 2 3 4; do
  ...
  echo "[nfs:remote] mount attempt failed on ${name}; client view of server exports:"
...
log "manager serves the export natively at $EXPORT_DIR - no self-mount"
...
for line in "${NODES[@]}"; do
  ...
  [ "${seen}" -eq "${total}" ] || fail=1
done

ssh_vm "$MGR_IP" 'sudo showmount -e localhost 2>/dev/null || sudo exportfs -v'
