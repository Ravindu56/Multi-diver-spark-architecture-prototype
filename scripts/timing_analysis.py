...
def analyse_kmeans(
    ...
    for r, rows in per_rank.items():
        rank_iter_times[r] = {
            int(row["iteration"]): _safe_float(row.get("iter_time_s", 0)) for row in rows
        }

    for agg in agg_rows:
        it = int(agg["iteration"])

        spark_time_mean = _safe_float(agg.get("spark_time_s_mean", agg.get("spark_time_s", 0)))
        sync_time_mean = _safe_float(agg.get("sync_time_s_mean", agg.get("sync_time_s", 0)))
        iter_time_mean = _safe_float(agg.get("iter_time_s_mean", agg.get("iter_time_s", 0)))
        sync_pct_mean = _safe_float(agg.get("sync_overhead_pct_mean", 0))
        shift_mean = _safe_float(agg.get("centroid_shift_mean", agg.get("centroid_shift", 0)))
        wcss_mean = _safe_float(agg.get("global_wcss_mean", agg.get("global_wcss", 0)))

        # spark_fraction: primary allocator decision signal
        spark_fraction = round(spark_time_mean / iter_time_mean, 4) if iter_time_mean > 0 else 0.0
        ...
        it_times_across_ranks = [
            rank_iter_times[r][it] for r in rank_iter_times if it in rank_iter_times[r]
        ]
        rank_variance_s = (
            round(max(it_times_across_ranks) - min(it_times_across_ranks), 6)
            if len(it_times_across_ranks) > 1
            else 0.0
        )
        ...
        if spark_time_mean == 0.0 and 0 in per_rank:
            ...
            if it in r0_rows:
                ...
                spark_fraction = (
                    round(spark_time_mean / iter_time_mean, 4) if iter_time_mean > 0 else 0.0
                )
                if spark_fraction > 0.6:
                    bottleneck = "compute"
                elif spark_fraction < 0.4:
...
def analyse_logreg(
    ...
    for r, rows in per_rank.items():
        rank_epoch_times[r] = {
            int(row["epoch"]): _safe_float(row.get("epoch_time_s", 0)) for row in rows
        }
    ...
    for row in ref_rows:
        ...
        sync_overhead_pct = round(sync_time / epoch_time * 100, 4) if epoch_time > 0 else 0.0
        spark_fraction = round(spark_time / epoch_time, 4) if epoch_time > 0 else 0.0
        ...
        # Rank variance
        ep_times = [rank_epoch_times[r][ep] for r in rank_epoch_times if ep in rank_epoch_times[r]]
        rank_variance_s = round(max(ep_times) - min(ep_times), 6) if len(ep_times) > 1 else 0.0
        ...
        grad_norm_decay = round(grad_norm / first_grad_norm, 6) if first_grad_norm > 0 else 0.0
...
def _bottleneck_mode(spark_fracs: list[float]) -> str:
    ...
    majority = max([(compute, "compute"), (sync, "sync"), (bal, "balanced")], key=lambda x: x[0])
    return majority[1]
...
def _print_iteration_table(rows: list[dict], workload: str, max_rows: int = 20) -> None:
    ...
    for row in rows[:max_rows]:
        line = "  ".join(f"{str(row.get(k, ''))[:18]:>18}" for k in keys)
        print("  " + line)
...
{'='*60}""")
    ...
    summary_fields = [
        ...
        "sync_overhead_pct_max",
        "spark_fraction_mean",
        "spark_fraction_std",
        "rank_variance_mean_s",
        "rank_variance_max_s",
        "bottleneck_mode",
    ]
    ...
    feature_matrix = build_feature_matrix(km_iter_detail, lr_epoch_detail, km_summary, lr_summary)
    if feature_matrix:
