...
a manifest row per run.  Two arms:
  ...
  throttled   - one worker rank is pinned to a reduced core set, inducing
                worker heterogeneity (research gap iii: sync-barrier
                behaviour under heterogeneous resource conditions).  The
                throttle is real: the worker's Spark local[N] budget is
                unchanged, so the pinned rank is genuinely over-subscribed.

Throttling methods (--throttle-method):

  taskset  - per-rank bash wrapper applying taskset(1) to the throttled
             rank (raw sched_setaffinity; works even on machines where
             OpenMPI's hwloc topology read fails).  DEFAULT.
  rankfile - OpenMPI rankfile slot binding (requires a healthy hwloc
             topology; on hosts where hwloc reports "invalid topology
             information" the slot inventory is degraded and rankfiles
             with higher slot ids are rejected at launch).
...
THROTTLE_WRAPPER = """#!/usr/bin/env bash
# Generated by run_sync_benchmark.py — pins one MPI rank to a reduced
# core set via taskset(1) (sched_setaffinity; no hwloc dependency, so it
# works even on machines where OpenMPI's hwloc topology read fails).
if [ "${OMPI_COMM_WORLD_RANK:-}" = "${THROTTLE_RANK:-1}" ]; then
  exec taskset -c "${THROTTLE_CORES:-0-1}" "$@"
fi
exec "$@"
"""
...
def render_rankfile(np_ranks, total_cores, throttle_rank=None, throttle_slots=2) -> str:
    ...
    set, that rank is pinned to throttle_slots cores.  Requires a healthy
    hwloc topology on the host - prefer the taskset method otherwise.
    """
...
def render_throttle_wrapper() -> str:
    """Content of the per-rank taskset wrapper script (throttled arm)."""
...
def build_command(
    ...
    throttle_rank: int = 1,
    throttle_cores: str = "0-1",
    python: str = sys.executable,
) -> list[str]:
    """mpirun command for one run.  Wired modes go through main_mpi;
    M3 runs via its standalone all-ranks collective module entry.  The
    throttled arm either binds via --rankfile or wraps each rank in the
    taskset wrapper (-x exports the throttle selectors to all ranks)."""
    cmd = ["mpirun", "--oversubscribe"]
    if rankfile_path:
        cmd += ["--rankfile", rankfile_path]
    if wrapper_path:
        cmd += ["-x", f"THROTTLE_RANK={throttle_rank}", "-x", f"THROTTLE_CORES={throttle_cores}"]
    cmd += ["-np", str(spec.np_)]
    if wrapper_path:
...
def main(argv=None):
    ...
    p.add_argument(
        "--throttle-method",
        default="taskset",
        choices=["taskset", "rankfile"],
        help="taskset: per-rank sched_setaffinity wrapper (hwloc-independent). "
        "rankfile: OpenMPI slot binding (needs a healthy hwloc topology).",
    )
    p.add_argument(
        "--throttle-rank",
        type=int,
        default=1,
        help="MPI rank to throttle in the throttled arm (rank 1 = worker 0).",
    )
    p.add_argument(
        "--throttle-cores",
        default="0-1",
        help="taskset core list for the throttled rank (taskset method).",
    )
    p.add_argument(
        "--throttle-slots",
        type=int,
        default=2,
        help="Slot count for the throttled rank (rankfile method).",
    )
    ...
    with open(manifest_path, "a", newline="", encoding="utf-8") as mf:
        ...
        for spec in plan:
            ...
            if spec.arm == "throttled":
                if args.throttle_method == "rankfile":
                    rf = render_rankfile(
                        spec.np_,
                        args.total_cores,
                    ...
                    wrapper_path = os.path.join(spec.run_dir, "throttle_wrapper.sh")
                    wp = Path(wrapper_path)
                    wp.write_text(render_throttle_wrapper(), encoding="utf-8")
                    wp.chmod(0o755)
            cmd = build_command(
                spec,
                args.input,
                args.logreg_iter,
                args.logreg_features,
                rankfile_path=rankfile_path,
                wrapper_path=wrapper_path,
                throttle_rank=args.throttle_rank,
                throttle_cores=args.throttle_cores,
            )
