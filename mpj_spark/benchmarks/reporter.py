...
def print_timing(tc: TimingCollector, worker_timings: list = None):
    ...
    if jvm_init > 0:
        print(
            f"  JVM Pre-warm   (T_Init) : {jvm_init:>8.4f} s  ({pct(jvm_init):>5.1f}% of total)  [barrier wait]"
        )
...
def print_comparison(multi_timing: dict, std_timing: dict):
    ...
    def _row(label, key, note=""):
        ...
        print(f"  {label:<32} {mv:>14.4f} {sv:>12.4f} {speedup:>8.2f}x  {flag}{suffix}")

    _row("Load Time (sec)", "load_time")
    _row("Avg Worker Proc Time (sec)", "processing_time", note="pure computation only")
