...
def dynamic_partition(input_path, num_partitions, output_dir):
    ...
    paths = fm.dynamic_partition()
    if len(paths) != num_partitions:
        raise RuntimeError(
            f"Expected {num_partitions} partitions, got {len(paths)}: {paths}"
        )
    return paths
...
def aggregate_logreg_results(
    ...
    if allreduce_result is not None:
        ...
        agg_mode = f"Allreduce — FedAvg per-iteration ({sync_mode})"
        ...
        agg_mode = f"Native MPI Collectives FedAvg ({sync_mode})"
    else:
        ...
        final_weights = avg_w.tolist()
        final_intercept = float(avg_intercept)
        agg_mode = f"Post-hoc row-weighted average — no sync ({sync_mode})"
...
def run_root(
    ...
    **_extra_kwargs,
    ...
    result_queue = Queue()
    timing_queue = Queue()
    ready_signals = [Process()._config.get("ctx", None) or Queue() for _ in range(num_workers)]  # standard event/queue
