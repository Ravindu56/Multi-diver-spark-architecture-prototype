...
def _print_comparison(
    ...
    baseline_threads=None, parity_iter=None, model_label=None,
):
    note_parts = []
    if baseline_threads:
    ...
    print(f'\n{SEP}')
    print(
        f'  Multi-Driver vs Baseline  |  app={app}  |  '
        f'workers={num_workers}{note}'
    )
...
def _print_timing_summary(
    ...
    seed_time=None, reassign_time=None, model_label=None,
):
    print(f'\n{DASH}')
    label = f'Timing Summary' + (f'  [{model_label}]' if model_label else '')
    print(f'  {label}')
    print(DASH)
...
def run_root(
    ...
    **_extra_kwargs,
    ...
    agg_mode = (
        ...
        else f'Queue FedAvg, {logreg_iter} iters  [M2]' if do_logreg_allreduce
        else f'Post-hoc row-weighted average  [M1]' if do_logreg_nosync
        else 'N/A'
    )
    ...
    if do_seed:
        ...
        seed_centres = compute_global_seed_centres(
            input_file=input_file, k=kmeans_k,
            total_cores=TOTAL_CORES, sample_fraction=0.05, seed=42,
        )
    ...
    if do_logreg_allreduce:
        ...
        allreduce_thread = threading.Thread(
            target=_allreduce_thread_fn, daemon=True
        )
    ...
    avg_init = (
        sum(t['init_time'] for t in worker_timings) / num_workers
    ...
    if compare:
        logreg_parity_iter = (
            num_workers * logreg_iter if app == 'logreg' else None
        )
        b_label = (
            'B2 — Standalone cluster' if baseline_master else 'B1 — local[N]'
        )
        _phase(
            'B',
