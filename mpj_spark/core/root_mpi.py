...
def run_root_mpi(
    ...
    agg_mode = (
        ...
        else f'Allreduce FedAvg MPI ({logreg_iter} iters)' if app == 'logreg'
        else 'N/A'
    )
    title_extra = ''
    if app == 'logreg':
    ...
    if do_reassign and agg is not None and app == 'kmeans':
        ...
        for i in range(num_workers):
            ...
            comm.send(
                ...
                dest=w_rank, tag=TAG_REASSIGN_BCAST,
            )
        ...
        corrected = [
            (all_sums[j] / all_counts[j]).tolist() if all_counts[j] > 0
        ...
        print(DASH)
        _ok(
            f'Re-assignment done  ({reassign_time:.3f}s)  '
