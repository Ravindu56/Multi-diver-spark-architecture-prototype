# P5-01 Companion — Low-RAM Host Tuning (14 GB Host)

Companion to `docs/p5-01-vm-swarm-setup.md`. The primary host runs the full
3-node Docker Swarm cluster (1 manager + 2 workers) as KVM VMs on a single
14 GB RAM machine. This document captures the host-side tuning that makes
the P5-01 topology fit safely, without converting the machine to a server
install.

## Sizing rule

Total VM RAM must stay at or below (available RAM - 2 GB). QEMU adds ~5-10%
overhead per VM on top of its configured RAM. Measured on the project host:
GUI idle footprint ~3 GB (11 GB available), 16 GB swap present.

| Host mode           | Usable for VMs | Per-VM RAM | 3-node total | Spark driver memory |
|---------------------|----------------|------------|--------------|---------------------|
| Desktop (GUI on)    | ~9-11 GB       | 2560 MB    | 7.5 GB       | 2g                  |
...
# pass to provision_swarm_vms.sh / virt-install as the per-VM memory
```
...
The desktop environment, not the package set, is what costs ~3-4 GB. Toggle
it per session with `scripts/host-mode.sh`:
...
the toggle yields the same headroom without losing the research workflow. A
quieter host also reduces CPU-scheduling noise in the Objective 2a profiling
data.

## OOM safety

- Keep swap enabled; it is the host's OOM insurance (16 GB swap file present
  on the project host).
