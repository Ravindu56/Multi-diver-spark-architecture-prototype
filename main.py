...
def _run_mpi_logreg(args, dataset_path):
    ...
    if baseline_timing is not None:
        ...
        for key, label in [
            ...
            m = mpi_elapsed if key == 'total_time' else mpi_elapsed * (0.9 if key == 'processing_time' else 0.1)
...
def main():
    ...
    if args.generate is not None:
        ...
        dataset_path = os.path.join(
            DATA_DIR,
            'numeric_dataset.csv' if args.app == 'kmeans'
            else 'classification_dataset.csv' if args.app == 'logreg'
    ...
    if args.baseline_master:
        ...
        print(f'[main] Baseline master  : local[N]  [B1 — use --baseline-master for B2]')
    if args.gossip:
        print(
            f'[main] Gossip           : ON  '
